        ...
        if (len(dn) == 0) and ('DATA_LOGGER' in dp):
            ...
            if (( -0.0 < tm ) & ( tm < 15.0)):
                ...
                if ciao == 1:
                    ...
                    format_str = """INSERT INTO CIAO_2_DataLoggers (timestamp, seeing, 
                             asm_seeing, strehl, tau0, alt, az, flux, tip_residuals, tilt_residuals)
                             VALUES ('{timestamp}', '{seeing}', '{asm_seeing}', '{strehl}', '{tau0}',
                             '{alt}', '{az}', '{flux}', '{tip}', '{tilt}');"""
                elif ciao == 3:
                    format_str = """INSERT INTO CIAO_3_DataLoggers (timestamp, seeing, 
                             asm_seeing, strehl, tau0, alt, az, flux, tip_residuals, tilt_residuals)
                             VALUES ('{timestamp}', '{seeing}', '{asm_seeing}', '{strehl}', '{tau0}',
                             '{alt}', '{az}', '{flux}', '{tip}', '{tilt}');"""
                elif ciao == 4:
                    format_str = """INSERT INTO CIAO_4_DataLoggers (timestamp, seeing, 
                             asm_seeing, strehl, tau0, alt, az, flux, tip_residuals, tilt_residuals)
                             VALUES ('{timestamp}', '{seeing}', '{asm_seeing}', '{strehl}', '{tau0}',
                             '{alt}', '{az}', '{flux}', '{tip}', '{tilt}');"""
