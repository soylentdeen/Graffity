...
def printPattern(current):
    ...
    print('sciapiSendCommand lci1hva QSPATTERN SET %s' % command)
...
while choice != 'Q':
    if choice == 'R':
        ...
        print("Here are the current .fits files in the current directory")
        print(glob.glob('*.fits'))
        filename = raw_input("Enter the name of the .fits from which to read the actuator positions:")
        current = pyfits.getdata(filename)
        current = numpy.append(current, -0.1032)
        current = numpy.append(current, 0.3281)
