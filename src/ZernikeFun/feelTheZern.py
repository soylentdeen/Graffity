...
class Map( object ):
    ...
    def applyCoefficients(self, coefficients):
        coeffs = numpy.zeros(len(self.ZernikeMatrix[0]))
        for i in range(numpy.min([len(coeffs), len(coefficients)])):
            coeffs[i] = coefficients[i]
        self.result = self.ZernikeMatrix.dot(coeffs)
    ...
    def save(self, name):
        hdu = pyfits.PrimaryHDU(data = numpy.array(self.result+3.5, dtype=numpy.float32))
