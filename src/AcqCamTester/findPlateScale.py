...
def findCorrelations(GravObs=[], ax=None):
    ...
    #    ax[1].clear()
    #    ax[2].clear()
    linex = numpy.array([numpy.ones(50), numpy.linspace(0, 0.5)])
    difference = {}
    strehl = {}
    cov = {}
    CIAO_SR = {}
        ...
        for Grav in GravObs:
            ...
            if (Grav.AcqCamData != None) :
                ...
                for SR, FT_flux, SC_flux, dx, dy, sc, pupil in zip(Acq.newStrehl.data[i],
                        ...
                        Acq.SC_Pos.data[i]):
                    if (not(numpy.isnan(SR)) and (SR > 0) and (SR < 2.5)):
                        if (FT_flux > -150000):
                            strehl[i].append(SR)
...
ax32 = fig3.add_axes([0.1, 0.5, 0.4, 0.4], sharex=ax31, sharey=ax31)
ax33 = fig3.add_axes([0.5, 0.1, 0.4, 0.4], sharey=ax31, sharex=ax31)
ax34 = fig3.add_axes([0.5, 0.5, 0.4, 0.4], sharex=ax33, sharey=ax32)
ax31.clear()
ax32.clear()
ax33.clear()
ax34.clear()
...
ax31.set_ylabel("N")
pyplot.setp(ax32.get_xticklabels(), visible=False)
pyplot.setp(ax34.get_xticklabels(), visible=False)
pyplot.setp(ax33.get_yticklabels(), visible=False)
pyplot.setp(ax34.get_yticklabels(), visible=False)
...
fig1.suptitle("Fringe Tracking Object: %s" % ftobj)
fig2.suptitle("Science Object: %s" % sobj)
fig3.suptitle("Strehl Ratio Frequency (%s)" % ftobj)
fig1.show()
fig2.show()
fig3.show()
#fig4.show()
#fig5.show()
fig1.savefig("FT_Flux_%s.png" % ftobj)
fig2.savefig("SC_Flux_%s.png" % sobj)
fig3.savefig("AcqCamSR_%s.png" % ftobj)
