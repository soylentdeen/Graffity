...
df = glob.glob(datadir+'GRAVI*_dualscip2vmred.fits')
...
def newImg(p):
    return p[0]*numpy.exp(-(xi-p[1])**2.0/p[2] - ((yi-p[3])**2.0/p[4]))

def calcEllipse(p, img):
    return (newImg(p).flatten() - img.flatten())**2.0
...
def fitEllipse(img):
    ...
    xc = 10.0
    yc = 10.0
    fit = leastsq(calcEllipse, [c, xc, rx, yc, ry], args=(img))
    ...
    try:
        ...
        for im, ftx, fty in zip(AcqCamImages, FTX.T, FTY.T):
            for i in Telescopes:
                ...
                ImageStack[i].append(im[ycoord-10:ycoord+10, xcoord-10:xcoord+10])
                ImageStack[i][-1] /= numpy.max(ImageStack[i][-1])
                el = fitEllipse(ImageStack[i][-1])
                strehl = Perfect.calcStrehl(im[ycoord-10:ycoord+10, xcoord-10:xcoord+10])
...
for i in Telescopes:
    ...
    ax.scatter(elipse[i], Strehl[i], c=colors[i-1])
